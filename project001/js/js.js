$(document).ready(function () {
  ...
  $('a').click(function (event) {
    if ($(this).attr('href') != '#' || $(this).parents('h1').length <= 0) {
  ...
  // 다크모드 버튼 클릭
  $('.util li').eq(3).click(function () {
    if ($('body').hasClass('day') == true) {
      $(':root').css({
        '--nightDay': '#f9f5ec'
      }).css({
        '--dayNight': '#514d47'
  ...
  $('.slide .imgBox>i').click(function () {
    ...
    let i = $(this).index()
    if (i == 7) {
      $('.slide .imgBox>a').eq(a - 1).css({
        'left': '-100%'
      }).stop().animate({
        'left': 0
      });
      $('.slide .imgBox>a').eq(a).css({
        'left': 0
      }).stop().animate({
          'left': '100%'
        },
  ...
  $('.news .tag li').click(function () {
    if ($(this).index() >= 3) {
      ...
      $('.listBox .box li').css({
        'order': 0
      }).removeClass('on').eq(newsi * 8).addClass('on');
      b = 0;
    };
  })
  ...
  $('.listBox .left i').click(function () {
    if (b <= 0) {
      b = 8;
      $('.listBox .box ul').eq(newsi).children('li').css({
        'order': 0
      });
    };
    $('.listBox .box ul').eq(newsi).css({
      'left': '-250px'
    }).stop().animate({
        'left': 0
      }, 100,
      function () {}).children('li').eq(b - 1).css({
      'order': -1
    });
    b--;
  });
  ...
  let noticeTimer = function () {
    ...
    $('.news .notice ul li').eq(c).css({
      'top': '100%'
    }).stop().animate({
      'top': '20%'
    });
    $('.news .notice ul li').eq(c - 1).css({
      'top': '20%'
    }).stop().animate({
      'top': '-60%'
    })
